...
declare global {
  interface Window {
    electronAPI: {
      ...
      mqttDisconnect: () => void;
      onMqttStatusUpdate: (callback: (status: string, error?: string) => void) => () => void;
      onJobNew: (callback: (job: Job) => void) => () => void;
      onJobUpdate: (callback: (update: { jobId: number; status: 'completed' | 'failed'; error?: string }) => void) => () => void;
    };
  }
}
...
const App: React.FC = () => {
  ...
  const [mqttStatus, setMqttStatus] = useState<'disconnected' | 'connecting' | 'connected' | 'error' | 'reconnecting'>('disconnected');
  const [jobs, setJobs] = useState<Job[]>([]);

  useEffect(() => {
    const unsubStatus = window.electronAPI.onMqttStatusUpdate((status, err) => {
      console.log(`[Renderer] Received MQTT status update: ${status}`, err || '');
  ...
  useEffect(() => {
    ...
    if (credentials.accessToken && credentials.username && credentials.password && credentials.tenantId && credentials.responderId) {
      console.log('[Renderer] Found existing credentials, requesting MQTT connection...');
      setView('home');
  ...
  const handleRegister = async (tenantId: string, enrollmentToken: string) => {
    ...
    try {
      ...
      const credentials = { accessToken, username, password, responderId: id, tenantId };
  ...
  return (
    <div className="app-container">
      {view === 'register' && (
        <RegisterScreen onRegister={handleRegister} loading={loading} error={error} />
      )}
      {view === 'home' && (
        <HomeScreen jobs={jobs} mqttStatus={mqttStatus} onLogout={handleLogout} />
      )}
    </div>
  );
};
